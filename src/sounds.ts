...
interface SoundDescriptor {
  ...
  relPath: string;
  /** Champ `src` prêt à copier dans storyboard.json (ex: "sounds/sfx/heartbeat.mp3"). */
  src: string;
...
async function main() {
  ...
  const descriptors: SoundDescriptor[] = [];
  ...
  if (descriptors.length === 0) {
    console.log(
      "ℹ️ Aucun son pour l'instant. Ajoute un fichier audio + son .md (voir public/sounds/_TEMPLATE.md)."
    );
  }
}
