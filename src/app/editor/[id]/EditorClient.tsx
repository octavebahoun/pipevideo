...
export default function EditorClient({ video }: EditorClientProps) {
  ...
  return (
    <div className="relative min-h-screen p-6 md:p-12 max-w-7xl mx-auto z-10">
      {/* Glow */}
      <div className="absolute top-0 left-10 w-96 h-96 bg-red-600/5 rounded-full blur-3xl animate-pulse-slow -z-10" />

      {/* Header */}
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8 border-b border-zinc-200 pb-6">
        <div className="flex items-center gap-4 min-w-0">
          <Link href="/dashboard" className="shrink-0 p-2.5 bg-white border border-zinc-200 hover:border-zinc-300 text-zinc-500 hover:text-zinc-900 rounded-xl transition-all">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-full min-w-0 bg-transparent text-xl sm:text-2xl font-bold text-zinc-900 border-b border-transparent hover:border-zinc-300 focus:border-red-500 focus:outline-none px-1"
              />
            </div>
            <p className="text-xs text-zinc-500 mt-1 truncate">Sujet original : {topic}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 bg-white hover:bg-zinc-100 disabled:opacity-50 text-zinc-900 rounded-xl font-medium border border-zinc-200 hover:border-red-700 transition-all duration-200"
            title="Analyser le script avec l'IA (OpenRouter)"
          >
            {isAnalyzing ? (
              <><Sparkles className="w-4 h-4 animate-pulse" /> Analyse en cours...</>
            ) : (
              <><Sparkles className="w-4 h-4 text-red-400" /> Analyser avec l'IA</>
            )}
          </button>
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 btn-glossy-red text-white rounded-xl font-medium transition-all duration-200"
          >
      ...
      {(analysis || analysisError) && (
        <section className="glass p-6 rounded-2xl border border-red-900/50 mb-8 relative">
          <button
            onClick={() => { setAnalysis(null); setAnalysisError(null); }}
            className="absolute top-4 right-4 text-zinc-500 hover:text-zinc-900 p-1 rounded-lg hover:bg-zinc-100 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
          <h3 className="text-md font-bold text-zinc-900 mb-3 flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-red-400" /> Analyse IA du script
          </h3>
          {analysisError ? (
            ...
            <div className="text-sm text-zinc-600 whitespace-pre-line leading-relaxed">{analysis}</div>
          )}
        </section>
      )}

      {/* Configuration Globale */}
      <section className="glass p-6 rounded-2xl border border-zinc-200 mb-8 grid grid-cols-1 md:grid-cols-4 gap-6">
        <div>
          <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Smile className="w-4 h-4 text-red-400" /> Voix Off (ElevenLabs)
          </label>
          <select 
            value={storyboard.voice || 'george'} 
            onChange={(e) => updateGlobalField('voice', e.target.value)}
            className="w-full bg-white border border-zinc-200 focus:border-red-500 rounded-xl px-3 py-2.5 text-sm text-zinc-900"
          >
        ...
        <div>
          <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Tv className="w-4 h-4 text-red-400" /> Format Vidéo
          </label>
          <select 
        ...
        <div>
          <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Music className="w-4 h-4 text-red-400" /> Musique de fond
          </label>
          <input 
            type="text" 
            ...
            className="w-full bg-white border border-zinc-200 focus:border-red-500 rounded-xl px-3 py-2 text-sm text-zinc-900 placeholder-zinc-400"
          />
        </div>
        ...
        <div>
          ...
          <input 
            ...
            className="w-full h-2 bg-zinc-200 rounded-lg appearance-none cursor-pointer accent-red-500 mt-3"
          />
        </div>
      </section>

      {/* YouTube Metadata */}
      <section className="glass p-6 rounded-2xl border border-zinc-200 mb-8">
        <h3 className="text-md font-bold text-zinc-900 mb-4 flex items-center gap-2">
          <Settings className="w-5 h-5 text-red-400" /> Métadonnées YouTube SEO
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-white border border-zinc-200 focus:border-red-500 rounded-xl px-4 py-2.5 text-sm text-zinc-900 placeholder-zinc-400 resize-y"
      ...
      <section className="glass p-6 rounded-2xl border border-zinc-200 mb-8">
        <h3 className="text-md font-bold text-zinc-900 mb-4 flex items-center gap-2">
          <CalendarClock className="w-5 h-5 text-red-400" /> Publication programmée
        </h3>
        <div className="flex flex-col md:flex-row md:items-end gap-4">
          <div className="flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full bg-white border border-zinc-200 focus:border-red-500 rounded-xl px-4 py-2.5 text-sm text-zinc-900 [color-scheme:light]"
            />
          </div>
          {scheduledFor && (
            <button
              ...
              className="px-4 py-2.5 bg-white hover:bg-zinc-100 text-zinc-500 hover:text-zinc-900 text-xs font-semibold rounded-xl border border-zinc-200 transition-colors"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <aside className="lg:col-span-4 glass p-6 rounded-2xl border border-zinc-200 flex flex-col h-[500px]">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-md font-bold text-zinc-900 flex items-center gap-2">
              <Layers className="w-5 h-5 text-red-400" /> Scènes ({storyboard.scenes.length})
            </h3>
            <button 
              onClick={addScene}
              className="flex items-center gap-1 px-3 py-1.5 btn-glossy-red text-white rounded-lg text-xs font-semibold transition-colors"
            >
              ...
              <div 
                ...
                className={`p-3.5 rounded-xl border cursor-pointer transition-all flex items-center justify-between ${
                  activeSceneId === scene.id 
                    ? 'bg-red-600/10 border-red-500 text-zinc-900' 
                    : 'bg-white/50 border-zinc-200 hover:border-zinc-300 text-zinc-500'
                }`}
              >
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-xs font-bold bg-zinc-100 px-2 py-1 rounded text-zinc-600">#{scene.id}</span>
                ...
                <div className="flex gap-1 items-center shrink-0">
                  <button 
                    ...
                    disabled={index === 0}
                    className="p-1 hover:bg-zinc-100 text-zinc-500 hover:text-zinc-900 rounded disabled:opacity-30"
                  >
                  ...
                  <button 
                    ...
                    className="p-1 hover:bg-red-50 text-zinc-500 hover:text-red-500 rounded"
                  >
        ...
        {/* Scene Editor Form */}
        <main className="lg:col-span-8 glass p-6 md:p-8 rounded-2xl border border-zinc-200 min-h-[500px]">
          {activeScene ? (
            <div className="space-y-6">
              <div className="flex justify-between items-center border-b border-zinc-200 pb-4">
                <h3 className="text-lg font-bold text-zinc-900">Édition Scène #{activeScene.id}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Narration (Edge-TTS)</label>
                  <textarea 
                    value={activeScene.narration} 
                    onChange={(e) => updateSceneField(activeScene.id, 'narration', e.target.value)}
                    rows={4}
                    className="w-full bg-white border border-zinc-200 focus:border-red-500 rounded-xl px-4 py-3 text-sm text-zinc-900 placeholder-zinc-400 resize-y"
                    placeholder="Saisissez la voix off de la scène..."
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Prompt Visuel (Seedance 1.5 Pro)</label>
                  <textarea 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Chemin Média (public/)</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Zoom</label>
                  <select 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Transition Entrée</label>
                  <select 
                ...
                <div>
                  <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Mouvement Caméra (Prompt IA)</label>
                  <select 
                ...
                <div className="flex items-center pt-6">
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-600">
                    <input 
                      ...
                      className="w-4 h-4 bg-white border-zinc-200 text-red-600 rounded focus:ring-red-500"
                    />
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <h3 className="text-lg font-medium text-zinc-900 mb-1">Aucune scène sélectionnée</h3>
